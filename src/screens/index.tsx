...
export default function Home() {
    ...
    useEffect(() => {
        const resizeCanvas = () => {
            ...
            if (canvas) {
                ...
                if (ctx) {
                    canvas.width = window.innerWidth;
                    canvas.height = window.innerHeight * 0.7; // Reduced to fit controls
                    ctx.fillStyle = 'black';
                    ctx.fillRect(0, 0, canvas.width, canvas.height);
                }
            }
    ...
    const preventTouchScroll = (e: React.TouchEvent<HTMLCanvasElement>) => {
        e.preventDefault();
    };
        ...
        <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-75">
            ...
            {latexExpression.length > 0 && (
                <div className="fixed inset-0 flex items-center justify-center z-30 pointer-events-none">
                    <div className="p-4 sm:p-3 md:p-4 text-black font-bold bg-white rounded w-1/4 text-center overflow-x-auto overflow-y-auto max-h-[80vh]">
                        <div className="latex-content break-words whitespace-pre-wrap text-xs sm:text-md md:text-base lg:text-lg">
                            {latexExpression[latexExpression.length - 1]}
                        </div>
                    </div>
